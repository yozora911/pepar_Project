...
bool Collision::SquareRayCast(const DirectX::XMFLOAT3& start, const DirectX::XMFLOAT3& end, const DirectX::XMFLOAT3& vertexA, const DirectX::XMFLOAT3& vertexB, const DirectX::XMFLOAT3& vertexC, const DirectX::XMFLOAT3& vertexD, DirectX::XMFLOAT3& hitPosition, DirectX::XMFLOAT3& hitNormal)
{
	...
	DirectX::XMVECTOR D = DirectX::XMLoadFloat3(&vertexD);

	float distance = 0.0f;

	//DirectXCollisionによる例と三角形の交差判定
	if (DirectX::TriangleTests::Intersects(Start, Direction, A, B, C, distance))
	{
		...
		DirectX::XMVECTOR lineA = DirectX::XMVectorSubtract(B, A);
		DirectX::XMVECTOR lineB = DirectX::XMVectorSubtract(A, C);
		DirectX::XMVECTOR Normal = DirectX::XMVector3Cross(lineA, lineB);
		DirectX::XMStoreFloat3(&hitNormal, Normal);
		return true;
	}
	if (DirectX::TriangleTests::Intersects(Start, Direction, B, C, D, distance))
	{
		DirectX::XMVECTOR HitPosition = DirectX::XMVectorAdd(Start, DirectX::XMVectorScale(Direction, distance));
		DirectX::XMStoreFloat3(&hitPosition, HitPosition);
